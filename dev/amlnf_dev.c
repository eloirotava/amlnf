...
#if 0 /* unused stub block ops; signatures changed in 6.x */
...
static struct class_attribute aml_version =
	__ATTR(version, S_IRUGO, NULL, NULL);
static struct class_attribute aml_part_table =
	__ATTR(part_table, S_IRUGO, NULL, NULL);
static struct class_attribute aml_store_device =
	__ATTR(store_device, S_IRUGO, NULL, NULL);
...
#endif
	{
		extern int amlnf_map_nfc(struct platform_device *pdev);
