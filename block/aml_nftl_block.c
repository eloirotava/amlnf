...
static int aml_nftl_init_bounce_buf(struct ntd_blktrans_dev *dev, struct request_queue *rq)
{
...
static int aml_nftl_reinit_part(struct aml_nftl_blk *nftl_blk)
{
...
static void aml_nftl_add_ntd(struct ntd_blktrans_ops *tr, struct ntd_info *ntd)
{
	...
	if (!amlnf_part_enabled(ntd->name)) {
		PRINT("amlnf: skipping partition %s (parts=%s)\n",
