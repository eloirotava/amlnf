...
static int uboot_env_open(struct inode *node, struct file *file)
{
	return 0;
}
static ssize_t uboot_env_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
...
static ssize_t uboot_env_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	unsigned char *env_ptr = NULL;
...
static long uboot_env_ioctl(struct file *file, unsigned int cmd, unsigned long args)
{
	return 0;
}
...
/* CLASS_ATTR API changed in 6.x — skip sysfs env attr for M3. */
...
int aml_ubootenv_init(struct amlnand_chip *aml_chip)
{
	...
	uboot_env_class = class_create(ENV_NAME);
	...
	/* class_attr_env skipped on 6.12 */
