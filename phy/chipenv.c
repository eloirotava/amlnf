...
extern struct amlnf_partition *amlnand_config;
static int get_last_reserve_block(struct amlnand_chip *aml_chip);
static int repair_reserved_bad_block(struct amlnand_chip *aml_chip);
...
static int amlnand_oops_handle(struct amlnand_chip *aml_chip)
{
	...
	unsigned int erase_shift, write_shift, writesize, erasesize, pages_per_blk;
...
*Note          :
*****************************************************************************/
static int amlnand_get_free_block(struct amlnand_chip *aml_chip, unsigned int block)
{
	struct hw_controller *controller = &aml_chip->controller;
	struct chip_operation *operation = &aml_chip->operation;
	struct chip_ops_para  *ops_para = &aml_chip->ops_para;
...
static void amlnand_info_error_handle(struct amlnand_chip *aml_chip)
{
...
int amlnand_read_info_by_name(struct amlnand_chip *aml_chip, unsigned char *info, unsigned char *buf, unsigned char *name, unsigned int size)
{
	struct hw_controller *controller = &aml_chip->controller;
	struct nand_flash *flash = &aml_chip->flash;
	struct chip_operation *operation = &aml_chip->operation;
	...
	unsigned int  offset, offset_tmp,  pages_per_blk, pages_read, amount_loaded = 0;
...
int amlnand_save_info_by_name(struct amlnand_chip *aml_chip, unsigned char *info, unsigned char *buf, unsigned char *name, unsigned int size)
{
	struct hw_controller *controller = &aml_chip->controller;
	struct nand_flash *flash = &aml_chip->flash;
	struct chip_operation *operation = &aml_chip->operation;
	...
	/*
	 * nbbt / ncnf / nkey / nenv.  The keys in particular are per unit and
	 * nothing can put them back, so this stays shut even when ordinary
	 * writes are allowed.
	 */
	if (!amlnf_allow_meta) {
		pr_warn("amlnf: readonly: refused to write metadata \"%s\"\n",
					   name ? (char *)name : "?");
		return -EROFS;
	}
...
int amlnand_check_info_by_name(struct amlnand_chip *aml_chip, unsigned char *info, unsigned char *name, unsigned int size)
{
	struct hw_controller *controller = &aml_chip->controller;
	struct nand_flash *flash = &aml_chip->flash;
	struct chip_operation *operation = &aml_chip->operation;
...
int amlnand_info_init(struct amlnand_chip *aml_chip, unsigned char *info, unsigned char *buf, unsigned char *name, unsigned int size)
{
	//struct hw_controller *controller = &aml_chip->controller;
	//struct nand_flash *flash = &aml_chip->flash;
	//struct chip_operation *operation = & aml_chip->operation;
...
static void amlnand_set_config_attribute(struct amlnand_chip *aml_chip)
{
