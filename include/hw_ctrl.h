...
/* Defined in nfc_map.c -- see the comments there. */
extern void __iomem *IO_NAND_BASE;
extern struct device *amlnf_dev;
extern int amlnf_readonly;
extern int amlnf_allow_markbad;
...
#ifdef CONFIG_NAND_AML_M8
/* Clock comes from DT clk framework; do not poke HHI via dead CBUS. */
 ...
 * otherwise: with the NFC clock off, or the chip in a state the driver did
 * not expect, the CPU spins forever with no output at all -- no oops, no
 * backtrace, just a dead box that the watchdog eventually reboots.  That is
 * exactly what the first M3 load looked like.  Every wait in this driver goes
 * through one of the two macros below.
 */
#define NFC_FIFO_SPIN_MAX	100000

#define NFC_WAIT_FIFO_EMPTY() do { \
	unsigned int __nfc_wait_n = 0; \
	while (NFC_CMDFIFO_SIZE() > 0 && ++__nfc_wait_n < NFC_FIFO_SPIN_MAX) \
		; \
} while (0)
