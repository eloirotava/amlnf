...
static enum hrtimer_restart controller_dma_timerfuc(struct hrtimer *timer)
{
	...
	unsigned int fifo_cnt = NFC_CMDFIFO_SIZE();
...
static int controller_hwecc_correct(struct hw_controller *controller, unsigned int size, unsigned char *oob_buf)
{
	...
	for (ecc_step_num = 0; ecc_step_num < (size / controller->ecc_unit); ecc_step_num++) {
		...
		usr_info = (*(unsigned int *)(&(controller->user_buf[ecc_step_num*info_times_int_len + user_offset])));
...
static int controller_dma_read(struct hw_controller *controller, unsigned int len, unsigned char bch_mode)
{
	...
	info_buf = (volatile unsigned int *)&(controller->user_buf[(count-1)*info_times_int_len]);
...
#else
	dcache_flush_range((unsigned int)controller->user_buf, count*PER_INFO_BYTE);
	dcache_invalid_range((unsigned int)controller->data_buf, len);
...
#else
	do {
		dcache_invalid_range((unsigned int)controller->user_buf, count*PER_INFO_BYTE);
		info_buf = (volatile unsigned int *)&(controller->user_buf[(count-1)*info_times_int_len]);
		cmp = *info_buf;
	} while ((cmp) == 0);
...
#else
	dcache_flush_range((unsigned int)controller->user_buf, count*PER_INFO_BYTE);
	dcache_flush_range((unsigned int)controller->data_buf, len);
	NFC_SEND_CMD_ADL((int)controller->data_buf);
	NFC_SEND_CMD_ADH((int)controller->data_buf);
	NFC_SEND_CMD_AIL((int)controller->user_buf);
	NFC_SEND_CMD_AIH((int)controller->user_buf);
...
static void controller_cmd_ctrl(struct hw_controller *controller, unsigned int cmd,  unsigned int ctrl)
{
