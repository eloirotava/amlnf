...
static int read_page(struct amlnand_chip *aml_chip)
{
	...
	unsigned int plane_page_addr, plane_blk_addr, pages_per_blk_shift, page_size, tmp_page;
...
static int write_page(struct amlnand_chip *aml_chip)
{
	...
	unsigned char *buf = ops_para->data_buf;
	unsigned char *oob_buf = ops_para->oob_buf;
	unsigned int plane_page_addr, plane_blk_addr, pages_per_blk_shift, page_size;
	unsigned int plane0_page_addr, plane1_page_addr, column, page_addr;
	unsigned char i, bch_mode, plane_num, chip_num, chipnr, user_byte_num;
	...
	if (amlnf_readonly) {
		amlnf_refuse_write("write_page", ops_para->page_addr);
		return -EROFS;
	}
...
static int set_blcok_status(struct amlnand_chip *aml_chip, unsigned char chipnr, unsigned int addr, int value)
{
	//struct hw_controller *controller = &aml_chip->controller;
	//struct nand_flash *flash = &aml_chip->flash;
...
static int get_blcok_status(struct amlnand_chip *aml_chip, unsigned char chipnr, unsigned int addr)
{
	//struct hw_controller *controller = &aml_chip->controller;
	//struct nand_flash *flash = &aml_chip->flash;
	unsigned int blk_addr = addr;
...
static int block_markbad(struct amlnand_chip *aml_chip)
{
	...
	if (!amlnf_allow_markbad) {
		amlnf_refuse_write("block_markbad", ops_para->page_addr);
		return -EROFS;
	}
...
static int erase_block(struct amlnand_chip *aml_chip)
{
	...
	if (amlnf_readonly) {
		amlnf_refuse_write("erase_block", ops_para->page_addr);
		return -EROFS;
	}
...
static int blk_modify_bbt_chip_op(struct amlnand_chip *aml_chip, int value)
{
	...
	if (!amlnf_allow_markbad) {
		amlnf_refuse_write("blk_modify_bbt", ops_para->page_addr);
		return -EROFS;
	}
