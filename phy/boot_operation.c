...
static int write_uboot(struct amlnand_phydev *phydev)
{
	...
	unsigned int configure_data, pages_per_blk, oobsize, page_size, tmp_size, priv_lsb, ops_tem;
	...
	for (i = 0; i < BOOT_COPY_NUM; i++) {
		...
		while (1) {
			if (((((unsigned int)addr / flash->pagesize)) % BOOT_PAGES_PER_COPY) == 0) {
			...
			ops_para->page_addr = ((unsigned int)addr / flash->pagesize);// /controller->chip_num;
 ...
 || ((ops_para->option & DEV_SLC_MODE) && ((unsigned int)addr%(flash->blocksize>>1) == 0)) \
							|| (((ops_para->option & DEV_SLC_MODE) == 0) && ((unsigned int)addr%flash->blocksize == 0))) {
				break;
			}
		}
	}
...
static int roomboot_nand_write(struct amlnand_phydev *phydev)
{
	struct amlnand_chip *aml_chip = (struct amlnand_chip *)phydev->priv;
	struct nand_flash *flash = &aml_chip->flash;
	struct phydev_ops *devops = &(phydev->ops);
	...
	write_len = ((((unsigned int)write_len + phydev->writesize)-1)/phydev->writesize)*phydev->writesize;
			 ...
			 size_t count, loff_t *ppos)
{
	...
	align_count = ((((unsigned int)count + phydev->writesize)-1)/phydev->writesize)*phydev->writesize;
